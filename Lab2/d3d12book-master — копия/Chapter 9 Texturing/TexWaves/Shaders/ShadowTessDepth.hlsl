...
[domain("tri")]
float4 DS(PatchTess patchTess, float3 bary : SV_DomainLocation, const OutputPatch<HullOut, 3> tri) : SV_POSITION
{
    float3 posL =
        bary.x * tri[0].PosL +
        bary.y * tri[1].PosL +
    ...
    posL += normalL * ((height - 0.5f) * 2.0f * gDisplacementScale);
